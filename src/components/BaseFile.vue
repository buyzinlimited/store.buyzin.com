...
<template>
  <div class="space-y-1 w-full">
    ...
    <!-- Upload Box -->
    <label
      class="w-full h-auto flex flex-col items-center justify-center border border-dashed border-gray-300 rounded-lg text-gray-400 hover:border-primary hover:text-primary cursor-pointer overflow-hidden relative transition p-1.5"
      :class="error ? 'border-red-500 text-red-500' : ''">
      <!-- Preview -->
      <template v-if="preview">
        <img :src="preview" alt="Preview" class="object-cover w-full h-full rounded-lg" />
